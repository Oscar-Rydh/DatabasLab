...
public class BookingPane extends BasicPane {
    ...
    private void fillNameList() {
        ...
        int i = 0;
        for (IndividualMovieInfo mv : db.getIndividualMovieInfo()){
            nameListModel.add(i, mv.move_name);
            i++;
        }
    }
    ...
    private void fillDateList() {
        ...
        //This is never used?
        //int i = 0;
        //for (MovieInfo mv : db.getAllMovieInfo()){
        //    System.out.println(mv.date);
        //    dateListModel.add(i, mv.date);
        //    i++;
        //}
    }
    ...
    class NameSelectionListener implements ListSelectionListener {
        ...
        public void valueChanged(ListSelectionEvent e) {
            ...
            dateListModel.removeAllElements();
            int i = 0;
            for (DateInfo di : db.getMovieDates(movieName)){
                dateListModel.add(i, di.date);
            }
        }
    }
    ...
    class DateSelectionListener implements ListSelectionListener {
        ...
        public void valueChanged(ListSelectionEvent e) {
            ...
            String movie_name = nameList.getSelectedValue();
            String date = dateList.getSelectedValue();
            fields[MOVIE_NAME].setText(movie_name);
            fields[PERF_DATE].setText(date);
            ScheduleInfo info = db.getScheduleInfo(movie_name, date);
            fields[THEATER_NAME].setText(info.theater_name);
            fields[FREE_SEATS].setText(Integer.toString(db.getFreeSeatsInfo(info.theater_name, movie_name, date)));
    ...
    class ActionHandler implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            String date = dateList.getSelectedValue();
            ScheduleInfo info = db.getScheduleInfo(movie_name, date);
            String theater = info.theater_name;
            String user = CurrentUser.instance().getCurrentUserId();
            db.makeReservation(user, theater, movie_name, date);
